...
Config.TOGGLES = {
    ...
    waypoint   = { label = "in-world waypoint", get = function() return Config.Waypoint().enabled ~= false end,
                   set = function(v) if ns.Waypoint then ns.Waypoint:SetEnabled(v) else Config.Waypoint().enabled = v end end },
    route      = { label = "dotted route line", get = function() return Config.Waypoint().route ~= false end,
