-- AUTO-GENERATED by tools/compile_guides.py from guides-src/GEN_ALLIANCE_DWARF_24_IRONFORGE.json - DO NOT EDIT
local _, ns = ...
ns.RegisterGuide({
    id = "GEN_ALLIANCE_DWARF_24_IRONFORGE",
    name = "24. Ironforge 44-44 (Dwarf)",
    version = 2,
    faction = "Alliance",
    race = { "Dwarf", "Gnome" },
    minLevel = 44,
    maxLevel = 44,
    map = 1455,
    zone = "Ironforge",
    next = "GEN_ALLIANCE_DWARF_25_SEARING_GORGE",
    author = "ForeverGuide route planner",
    notes = "Chapter 24 of the Dwarf route: level 44 to 44, 9 steps, ~22 min of play in the model (44470 xp/h). Route tuned for xp per hour: low-value quests, elites and long escorts are skipped on purpose.",
    steps = {
        { type = "TRAVEL", map = 1455, zone = "Ironforge", x = 61.0, y = 47.4, radius = 60, note = "travel to Ironforge (Ironforge)" }, -- 1
        { type = "ACCEPT", quest = 1050, questName = "Mythology of the Titans", npc = 3979, npcName = "Librarian Mae Paledust", map = 1455, zone = "Ironforge", x = 75, y = 12.5 }, -- 2
        { type = "ACCEPT", quest = 3448, questName = "Passing the Burden", npc = 2916, npcName = "Historian Karnik", map = 1455, zone = "Ironforge", x = 77.5, y = 11.8 }, -- 3
        { type = "TURNIN", quest = 1050, questName = "Mythology of the Titans", npc = 3979, npcName = "Librarian Mae Paledust", map = 1455, zone = "Ironforge", x = 75, y = 12.5, note = "reduced xp (80%) - you out-levelled it" }, -- 4
        { type = "TURNIN", quest = 3448, questName = "Passing the Burden", npc = 8507, npcName = "Tymor", map = 1455, zone = "Ironforge", x = 31, y = 4.8 }, -- 5
        { type = "ACCEPT", quest = 3450, questName = "An Easy Pickup", npc = 8507, npcName = "Tymor", map = 1455, zone = "Ironforge", x = 31, y = 4.8 }, -- 6
        { type = "TURNIN", quest = 3450, questName = "An Easy Pickup", npc = 8517, npcName = "Xiggs Fuselighter", map = 1455, zone = "Ironforge", x = 70.9, y = 94.6 }, -- 7
        { type = "ACCEPT", quest = 3451, questName = "Signal for Pickup", npc = 8517, npcName = "Xiggs Fuselighter", map = 1455, zone = "Ironforge", x = 70.9, y = 94.6 }, -- 8
        { type = "TURNIN", quest = 3451, questName = "Signal for Pickup", npc = 8517, npcName = "Xiggs Fuselighter", map = 1455, zone = "Ironforge", x = 70.9, y = 94.6 }, -- 9
    },
})
