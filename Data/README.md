...
(wago.tools export), `rxp` = factual positions (quest giver / turn-in NPC, objective areas) taken from
the free WoW Forever guides shipped with **RestedXP Guides** (https://github.com/RestedXP/RXPGuides,
licensed CC BY-NC-SA 4.0). Only the world facts are used - where an NPC stands, where the mobs are -
never their route or texts; ForeverGuide's routes are computed by `tools/plan_route.lua`.
